...
<script type="text/babel">
   ...
   export default {
        ...
        ready () {
            ...
            localStorage.setItem("velopedia_current_tab", '/inspiration');
        ...
        methods: {
            fetchItems (offset, size, func) {
                ...
                this.$http.get(this.apiURL, function (results, status, request) {
                    ...
                    let all = $(".tile").map(function(i, item) {
                        return {
                            reblogKey: $(item).attr('data-tumblrreblogkey'),
                            id: $(item).attr('data-tumblrid')
                        }
                    }).get();
                    //debugger;
                    ...
                    results.forEach( i => {
                        all.forEach( j => {
                            //we have a duplicate
                            if(j.reblogKey == i.reblog_key && j.id != i.id) {
