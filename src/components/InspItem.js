...
var Insp = Vue.extend({
    ...
    computed: {
        url: function () {
            ...
            if (_.find(this.item.photos[0].alt_sizes, function (size) {
                    return size.width === 400;
                })) {
                imgUrl = _.find(this.item.photos[0].alt_sizes, function (size) {
                    return size.width === 400;
                }).url;
            } else if (_.find(this.item.photos[0].alt_sizes, function (size) {
                    return size.width === 399
                })) {
                imgUrl = _.find(this.item.photos[0].alt_sizes, function (size) {
                    return size.width === 399
                }).url;
